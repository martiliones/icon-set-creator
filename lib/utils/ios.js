...
const iosConfigFile = 'ios/Runner.xcodeproj/project.pbxproj';
const iosAssetFolder = 'ios/Runner/Assets.xcassets/';
...
const changeIosLauncherIcon = (iconName, flavor) => {
  return new Promise((resolve, reject) => {
    fs.readFile(iosConfigFile, 'utf-8', (err, config) => {
      ...
      for (let i = 0; i <= lines.length; i++) {
        ...
        if (onConfigurationSection) {
          ...
          if (currentConfig &&
              (flavor || currentConfig.includes(`-${flavor}`)) &&
              line.includes('ASSETCATALOG')) {
...
const modifyContentsFile = (newCatalogName, newIconName) => {
  const newIconDirectory = path.resolve(iosAssetFolder, newCatalogName + '.appiconset/Contents.json');
