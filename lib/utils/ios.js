...
exports.createIosIcons = (imagePath, ios, flavor, context) => {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(imagePath, async (err, image) => {
      ...
      await changeIosLauncherIcon(catalogName, projectName, flavor);
...
const changeIosLauncherIcon = (iconName, projectName, flavor) => {
  return new Promise((resolve, reject) => {
    fs.readFile(getIosConfigFile(projectName), 'utf-8', (err, config) => {
      if (err) {
        if (err.code === 'ENOENT') {
      ...
      resolve(fs.writeFileSync(getIosConfigFile(projectName), entireFile));
    });
  });
};
