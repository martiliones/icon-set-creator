  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(imagePath, async (err, image) => {
      ...
      for (const iosIcon of iosIcons) {
        await saveIcon(image, path.resolve(context, getIosDefaultIconFolder(projectName, flavor)), iconName, iosIcon);
      }

      await changeIosLauncherIcon(catalogName, context, projectName);

      modifyContentsFile(catalogName, iconName, projectName, context);
...
const modifyContentsFile = (newCatalogName, newIconName, projectName, context) => {
  const newIconDirectory = path.resolve(context, getIosAssetFolder(projectName), newCatalogName + '.appiconset/Contents.json');
