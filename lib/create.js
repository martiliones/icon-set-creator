...
const { resolveConfig, error, info, log } = require('../utils/index');
...
const create = async (...args) => {
  try {
    ...
    if (!imagePath) {
      const iconset = resolveConfig(context);
    ...
    if (options.ios) {
      ...
      await createIosIcons(imagePathIos, options.ios, options.flavor, context);
    }
